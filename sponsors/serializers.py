...
class SponsorCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if attrs.get("is_organization") and not attrs.get("organization_name"):
            raise ValidationError({"warning": "Organization name must be set."})
        return attrs
    ...
    def validate_phone(self, phone: str):  # noqa
        ...
        raise ValidationError(
            [{"warning": "Phone number must be entered in the format:  " "+998 99 999 99 99 .Up to 15 digits allowed."}]
        )
